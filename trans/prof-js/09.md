...
![Figure 9.1: Traditional programming approach ](img/C14587_09_01.jpg)
...
当我们的应用变得越来越复杂时，我们的`Agent`类也变得越来越复杂。 由于它具有`constructor`中的所有引用，我们可能会遇到错误地传递参数类型所导致的问题。 当我们试图同时在多个组件之间进行通信时，这是一个常见的问题。
...
![Figure 9.2: The observer pattern ](img/C14587_09_02.jpg)
...
![Figure 9.3: Observer patterns using events ](img/C14587_09_03.jpg)
...
5.  Emit another event:
    ...
    ![Figure 9.4: Emitted event with the correct event value ](img/C14587_09_04.jpg)
...
7.  Emit another event:
    ...
    ![Figure 9.5: Output after emitting an event multiple times ](img/C14587_09_05.jpg)
...
10.  Emit the new event type:
    ...
    ![Figure 9.6: Emitting the new event type  ](img/C14587_09_06.jpg)
...
![Figure 9.7: Output of the emit event after the listener has been removed ](img/C14587_09_07.jpg)
...
![Figure 9.8: Listener invoked three times with the emit event before removing the listener ](img/C14587_09_08.jpg)
...
![Figure 9.9: After using removeListener, the first listener is removed ](img/C14587_09_09.jpg)
...
![Figure 9.10: Using removeAllListeners will output nothing ](img/C14587_09_10.jpg)
...
![Figure 9.11: Output after using the handleEvent listener ](img/C14587_09_11.jpg)
...
![Figure 9.12: Getting a one-time listener using the .once method ](img/C14587_09_12.jpg)
...
![Figure 9.13: Output showing the number of events attached to event 1 ](img/C14587_09_13.jpg)
...
![Figure 9.14: Output showing the number of events attached to event 2 ](img/C14587_09_14.jpg)
...
![Figure 9.15: Getting a list of listeners attached to the event using the EventEmitter class ](img/C14587_09_15.jpg)
...
![Figure 9.16: Checking whether the handler is attached to the emitter ](img/C14587_09_16.jpg)
...
![Figure 9.17: Getting information on event names using the EventEmitter class ](img/C14587_09_17.jpg)
...
![Figure 9.18: Warning when attaching more than 10 listeners for a single event ](img/C14587_09_18.jpg)
...
![Figure 9.19: Emitting the event after attaching the second event before the first one ](img/C14587_09_19.jpg)
...
![Figure 9.20: Ordering the event using prependListener ](img/C14587_09_20.jpg)
...
![Figure 9.21: Defining the fire alarm class ](img/C14587_09_21.jpg)
...
![Figure 9.22: An error is thrown when we run the code with the updated FireAlarm ](img/C14587_09_22.jpg)
...
![Figure 9.23: Event listener for the 'low-battery' event triggered correctly ](img/C14587_09_23.jpg)

###### 图 9.23:正确触发“低电量”事件的事件监听器

在这里，我们可以看到我们像处理常规的`EventEmitter`一样处理`livingRoomAlarm`，当我们发出*低电量*事件时，我们看到该事件的事件监听器被正确触发。 在下一个练习中，我们将使用我们所学的`EventEmitters`来制作一个非常简单的聊天室应用。
...
7.  Print the message log for `bob`:
    ...
    ![Figure 9.24: Printing the message log for bob ](img/C14587_09_24.jpg)
...
8.  Print the message log for `kevin`:
    ...
    ![Figure 9.25: Printing the message log for kevin ](img/C14587_09_25.jpg)
...
10.  Check the message log for `kevin`:
    ...
    ![Figure 9.26: Checking the message log for kevin ](img/C14587_09_26.jpg)
...
15.  Check the message list for `bob`:
    ...
    ![Figure 9.27: Checking the message list for bob ](img/C14587_09_27.jpg)
...
![Figure 9.28: Checking the message list for kevin again ](img/C14587_09_28.jpg)
...
在前一节中，我们广泛地学习了`events`模块，并学习了关于在应用中使用事件实现简单通信的所有内容。 `events`模块是 Node.js 提供的内置模块，不需要使用`npm`安装。 在本模块中，我们将讨论如何使用`fs`，`path`和`util`模块。
...
当我们在应用中处理目录和文件时，`Path.join()`是一个非常有用的函数。 它允许我们将路径连接在一起，并输出一个可以在**fs**模块中使用的路径字符串。 要使用`join`路径，我们可以调用`join`方法，并为其提供一个路径列表。 让我们看看下面的例子:
...
![Figure 9.29: Using the path.join function to combine different paths ](img/C14587_09_29.jpg)
...
![Figure 9.30: Showing the parent directory of our current directory ](img/C14587_09_30.jpg)
...
![Figure 9.31: File path parsed using the path.parse function ](img/C14587_09_31.jpg)
...
![Figure 9.32: Combining information into a single string path using path.format ](img/C14587_09_32.jpg)
...
**fs**模块是一个内置模块，为您提供 api，以便您可以与主机文件系统进行交互。 当我们需要在应用中处理文件时，它非常有用。 在本节中，我们将讨论如何在应用中使用`async`和`await`的**fs**模块。 稍后，我们将讨论最近添加的`fs.promises`API，它提供了相同的功能，但返回一个承诺而不是使用回调。
...
![Figure 9.33: Output after using the fs.stat method ](img/C14587_09_33.jpg)
...
![Figure 9.34: Reading the entire content of a file using the fs.readFile function ](img/C14587_09_34.jpg)
...
![Figure 9.35: Reading the entire content of a file using the fs.readFile function after encoding ](img/C14587_09_35.jpg)
...
在本练习中，我们将使用`fs`模块在应用中读写文件。 我们将使用前一节中介绍的方法，并将它们与回调一起使用。 然后，我们将`promisify`和`async`、`await`放在一起使用。
...
1.  Create a new file called `test.txt`:
    ...
    ![Figure 9.36: New test.txt file created ](img/C14587_09_36.jpg)

    ###### 图 9.36:新建 test.txt 文件

    你应该能够在与源代码相同的目录中看到新文件:

    ![Figure 9.37: New file created in the same directory as your source code ](img/C14587_09_37.jpg)
...
2.  Read its contents and output it in the console:
    ...
    ![Figure 9.38: Reading the file's content using fs.readFile ](img/C14587_09_38.jpg)
...
3.  Try to read from a file that doesn't exist:
    ...
    ![Figure 9.39: Error thrown when we try to read a file that doesn't exist ](img/C14587_09_39.jpg)
...
4.  Let's create our own version of `readFile` with promises:
    ...
    ![Figure 9.40: Creating a readFile using callback-based methods ](img/C14587_09_40.jpg)
...
![Figure 9.41: Calling the promise counterpart by importing fspromise ](img/C14587_09_41.jpg)
...
![Figure 9.42: Using the async function as a callback-based function ](img/C14587_09_42.jpg)
...
![Figure 9.43: Using the async function by calling the callbackify function  ](img/C14587_09_43.jpg)
...
![Figure 9.44: The promisify function is used for callbacks ](img/C14587_09_44.jpg)

###### 图 9.44:promise 函数用于回调

在`util`模块中还有许多类型检查方法，当我们试图在应用中找出变量的类型时，这些方法非常有用。

### 【定时器 T0】

timer 模块为我们提供了一个用于调度计时器函数的 API。 我们可以使用它来设置部分代码的延迟，或者按所需的时间间隔执行代码。 与之前的模块不同，`timer`模块在使用之前不需要导入。 让我们看看 Node.js 中提供的所有计时器函数，以及如何在应用中使用它们。
...
![Figure 9.45: Setting up a repeatedly executing function using the setInterval function ](img/C14587_09_45.jpg)
...
![Figure 9.46: Setting a one-time delayed invocation of a function using the setTimeout function ](img/C14587_09_46.jpg)
...
![Figure 9.47: Function to be executed at the end of the event loop pushed using setimmediate ](img/C14587_09_47.jpg)
...
![Figure 9.48: Stopping the interval from running using the clearInterval function ](img/C14587_09_48.jpg)
...
![Figure 9.49: Using setTimeout to wrap clearInterval within the specified seconds ](img/C14587_09_49.jpg)
...
在本章中，我们讨论了 JavaScript 中的事件系统，以及如何使用内置的`events`模块来创建我们自己的事件发射器。 稍后，我们讨论了一些有用的内置模块和它们的示例用法。 在编写需要多个组件相互通信的程序时，使用事件驱动编程可以帮助我们避免交错逻辑。 此外，通过使用内置模块，我们可以避免添加提供相同功能的模块，并避免创建具有巨大依赖性的项目。 我们还提到了如何使用计时器来控制程序执行，`fs`来操作文件，`path`来组合并获取关于文件路径的有用信息。 这些都是非常有用的模块，可以在以后构建应用时帮助我们。 在下一章中，我们将学习如何在 JavaScript 中使用函数式编程。