...
在用最大似然算法处理文本之前标记文本的方式对算法的性能有很大影响。许多 NLP 和 ML 应用程序使用*单词包*方法，其中只有单词或标记重要，但它们的顺序不重要，正如我们在[第 5 章](05.html)、*分类算法*中探索的朴素贝叶斯分类器。然而，生成*二元模型*的标记器，或者彼此相邻的词对，实际上将保留原始文本的一些位置和语义意义，即使与词包算法一起使用。
