...
**模板文字**是在 ECMAScript 6 中引入的一种新的字符串形式。 它们由**反勾**符号(````)括起来，而不是通常的单引号或双引号。 模板字面量允许您在运行时计算的字符串中嵌入表达式。 因此，我们可以很容易地从变量和变量表达式中创建动态字符串。 这些表达式用美元符号和花括号(`${ expression }`)表示。 模板字面量语法如下面的代码所示:
...
![Figure 1.16: Transpiled script output ](image/Figure_1.16.jpg)
...
![Figure 1.17: Calling the generator output ](image/Figure_1.17.jpg)
...
![Figure 1.18: Implementing generators output ](image/Figure_1.18.jpg)
