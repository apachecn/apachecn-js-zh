...
在这一章中，我们利用在第二章和第三章中获得的知识，构建了一个智能气象站的原型。我们从识别建造气象站所需的传感器开始。接下来，我们将它们设置在树莓 Pi 3 上。我们编写了与传感器接口所需的代码。完成后，我们更新了应用编程接口引擎，从树莓 Pi 3 中读取关于新主题的数据。一旦应用编程接口引擎接收到数据，我们就将其保存在数据库中，然后通过网络套接字将其发送到网络、桌面和移动应用。最后，我们更新了网络、桌面和移动应用上的演示模板；然后，我们在网络、桌面和移动应用上显示了树莓皮的数据。
