...
保存所有文件，启动代理、应用编程接口引擎和网络应用程序，然后登录应用程序，然后导航到设备页面。
...
我们在 web 应用程序中编写的所有代码最终都被捆绑到前面的文件中。我们将抓取`dist`文件夹中的所有文件(不是`dist`文件夹)，然后将其粘贴到`desktop-app/app`文件夹中。经过上述更改后，桌面应用程序的最终结构如下:
