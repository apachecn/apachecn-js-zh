...
在这一章中，我们看到了两种使用树莓皮的监控方法。第一种方法是我们将图像流式传输到应用编程接口引擎，然后使用 MJPEG 在移动网络和桌面应用上可视化图像。第二种方法是检测运动，然后开始录制视频。然后通过电子邮件将此视频作为附件发送。这两种方法也可以结合在一起，并且如果在方法一中检测到运动，则可以开始 MJPEG 流。
