...
这一章是关于基础的。我们将指导您如何编写您的第一个规范，首先在测试中思考，并展示所有可用的全局 Jasmine 函数。到这一章结束时，你应该知道Jasmine是如何工作的，并准备好开始自己做第一次测试。
...
在使用设置和拆卸功能时，Jasmine也尊重外部设置和拆卸功能，所以它们按预期运行。对于每个规范(`it`):
...
Jasmine匹配器只是一个返回布尔值的函数:`true`，表示期望通过，否则为`false`。
...
改变之后，规范应该还会通过。但是如果一个规范失败了会发生什么呢？Jasmine报告的错误信息是什么？
