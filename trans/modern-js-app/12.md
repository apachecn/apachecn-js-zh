...
**阴影 DOM** 允许 HTML 元素获得一种新的Node，称为与其关联的阴影根。有关联的阴影根的元素称为阴影宿主。不呈现阴影宿主的内容；而是呈现阴影根的内容。一个阴影根下面可以有另一个阴影根。
...
现在，每当这些异步活动发生时，它都会检查所有可能改变的东西，包括对象属性和来自根Node的所有组件的`this`关键字的数组元素；如果检测到任何变化，那么组件的模板绑定将被更新。Angular 2 不会简单地重新渲染整个组件。相反，它会检查已更改的绑定，并专门选择和更新它们。
