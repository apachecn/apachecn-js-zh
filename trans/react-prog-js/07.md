...
然而，不可变. js 的数据结构不仅是不可变的；他们在某些方面也很懒惰，文档清楚地标记了应用的哪些方面是迫切需要的。(提醒一下，当需要时，会以按需打印的方式处理惰性数据结构，而急切的操作会立即提前完成)。此外，某些函数式习惯用法被烘焙成不可变的. js 工具。例如，提供了一种.`take(n)`方法。它以经典的功能方式返回列表中的第一个 *n* 项。其他功能性订书钉，如`map()`、`filter()`和`reduce()`也有。总的来说，运行时的复杂性是计算机科学家合理要求的。
