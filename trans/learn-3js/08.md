...
我们之前已经看过了`JSONLoader`，但是这次用的是`load`功能，而不是`parse`功能。在这个函数中，我们指定要加载的 URL(指向导出的 JSON 文件)、加载对象时调用的回调，以及可以找到纹理的位置`../img/models/`(相对于页面)。该回调采用两个参数:`geometry`和`mat`。`geometry`参数包含模型，`mat`参数包含一组材质对象。我们知道只有一种材料，所以我们在创建`THREE.Mesh`的时候，直接引用那种材料。如果打开`05-blender-from-json.html`示例，可以看到我们刚从 Blender 导出的椅子。
