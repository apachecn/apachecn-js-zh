...
在这一章中，我们将看几个不同的方法，你可以创建高级和复杂的几何图形和网格。在[第 5 章](05.html "Chapter 5. Learning to Work with Geometries")、*学习使用几何图形*和[第 6 章](06.html "Chapter 6. Advanced Geometries and Binary Operations")、*高级几何图形和二进制运算*中，我们向您展示了如何使用 Three.js 中的内置对象创建一些高级几何图形。在本章中，我们将使用以下两种方法来创建高级几何图形和网格:
...
我们从*组开始，合并*方法。通过这种方法，我们使用标准的三. js 分组和`THREE.Geometry.merge()`函数来创建新的对象。
...
![Grouping objects together](graphics/2215OS_08_01.jpg)
...
![Grouping objects together](graphics/2215OS_08_02.jpg)
...
![Merging multiple meshes into a single mesh](graphics/2215OS_08_03.jpg)
...
![Merging multiple meshes into a single mesh](graphics/2215OS_08_04.jpg)
...
Three.js 可以读取多种 3D 文件格式，并导入这些文件中定义的几何图形和网格。下表显示了三个 js 支持的文件格式:
...
描述
 ...
 |
| --- | --- |
| JSON | 三. js 有自己的 JSON 格式可以用来声明定义一个几何图形或者场景。尽管这不是官方格式，但它非常容易使用，当您想要重用复杂的几何图形或场景时，它会非常方便。 |
...
| 科拉达 | Collada 是一种格式，用于以基于 XML 的格式定义*数字资产*。这也是几乎所有 3D 应用程序和渲染引擎都支持的一种广泛使用的格式。 |
...
对于三个 js 中的两个不同场景，可以使用三个 js 的 JSON 格式，可以使用它保存和加载单个`THREE.Mesh`，也可以使用它保存和加载一个完整的场景。
...
![Saving and loading THREE.Mesh](graphics/2215OS_08_05.jpg)
...
正如你所看到的，三个 js 保存了所有关于`THREE.Mesh`的信息。要使用 HTML5 本地存储 API 保存这些信息，我们所要做的就是调用`localStorage.setItem`函数。第一个参数是键值(`json`)，稍后我们可以使用它来检索作为第二个参数传入的信息。
...
![Saving and loading a scene](graphics/2215OS_08_06.jpg)
...
传入加载器的最后一个参数(`'.'`)定义了相对网址。例如，如果你有使用纹理的材料(例如，外部图像)，这些将使用这个相对网址检索。在这个例子中，我们不使用纹理，我们只是传入当前目录。就像`THREE.ObjectLoader`一样，你也可以使用`load`功能从一个网址加载一个 JSON 文件。
...
![Working with Blender](graphics/2215OS_08_07.jpg)

### 在搅拌机中安装三个 js 导出器

要让 Blender 导出三. js 模型，我们首先需要将三. js 导出器添加到 Blender 中。以下步骤适用于苹果 OS X，但在视窗和 Linux 上基本相同。您可以从[www.blender.org](http://www.blender.org)和下载 Blender，并按照特定平台的安装说明进行操作。安装后，您可以添加三. js 插件。首先，使用终端窗口从您的 Blender 安装中找到`addons`目录:

![Installing the Three.js exporter in Blender](graphics/2215OS_08_08.jpg)
...
接下来，您需要获取三. js 发行版，并在本地将其解包。在这个分布中，可以找到以下文件夹:`utils/exporters/blender/2.65/scripts/addons/`。在这个目录中，有一个名为`io_mesh_threejs`的子目录。将此目录复制到搅拌机安装的`addons`文件夹中。

现在，我们需要做的就是启动 Blender 并启用导出器。在搅拌机中，打开**搅拌机用户首选项** ( **文件** | **用户首选项**)。在打开的窗口中，选择**插件**选项卡，在搜索框中输入`three`。这将显示以下屏幕:

![Installing the Three.js exporter in Blender](graphics/2215OS_08_09.jpg)

此时点，找到三. js 插件，但仍被禁用。选中右边的小复选框，将启用三个. js 导出器。最后检查一下是否一切正常，打开**文件** | **导出**菜单选项，你会看到三个. js 被列为导出选项。这显示在下面的截图中:

![Installing the Three.js exporter in Blender](graphics/2215OS_08_10.jpg)

安装插件后，我们可以加载第一个模型。

### 从 Blender 加载和导出模型

作为一个的例子，我们在`assets/models`文件夹中添加了一个名为`misc_chair01.blend`的简单 Blender 模型，你可以在这本书的来源中找到。在本节中，我们将加载这个模型，并展示将这个模型导出到 Three.js 所需的最少步骤

首先，我们需要在 Blender 中加载这个模型。使用**文件** | **打开**并导航到包含`misc_chair01.blend`文件的文件夹。选择该文件，点击**打开**。这将向您显示一个屏幕，看起来有点像这样:

![Loading and exporting a model from Blender](graphics/2215OS_08_11.jpg)

将这个模型导出为三. js JSON 格式非常简单。在**文件**菜单中，打开**导出** | **三. js** ，输入导出文件的名称，选择**导出三. js** 。这将创建一个格式为 Three.js 理解的 JSON 文件。该文件的部分内容如下所示:
...
![Loading and exporting a model from Blender](graphics/2215OS_08_12.jpg)
...
我们之前已经看过了`JSONLoader`，但是这次用的是`load`功能，而不是`parse`功能。在这个函数中，我们指定要加载的 URL(指向导出的 JSON 文件)、加载对象时调用的回调，以及可以找到纹理的位置`../assets/models/`(相对于页面)。该回调采用两个参数:`geometry`和`mat`。`geometry`参数包含模型，`mat`参数包含一组材质对象。我们知道只有一种材料，所以我们在创建`THREE.Mesh`的时候，直接引用那种材料。如果打开`05-blender-from-json.html`示例，可以看到我们刚从 Blender 导出的椅子。

使用三. js 导出器不是将模型从 Blender 加载到三. js 中的唯一方法。三. js 了解许多 3D 文件格式，Blender 可以以其中的几种格式导出。然而，使用 Three.js 格式非常容易，如果出现问题，通常会很快被发现。

在下一节中，我们将研究 Three.js 支持的几种格式，并展示一个基于 Blender 的 OBJ 和 MTL 文件格式的例子。

## 从 3D 文件格式导入

在本章的开头，我们列出了三个 js 支持的许多格式。在这一节中，我们将快速浏览这些格式的几个示例。请注意，对于所有这些格式，需要包含一个额外的 JavaScript 文件。您可以在`examples/js/loaders`目录中的 Three.js 发行版中找到所有这些文件。

### OBJ 和 MTL 模式

OBJ 和 MTL 是相伴格式，经常一起使用。 OBJ 文件定义几何图形，MTL 文件定义使用的材料。OBJ 和 MTL 都是基于文本的格式。OBJ 文件的一部分是这样的:
...
![The OBJ and MTL formats](graphics/2215OS_08_13.jpg)
...
通常，一个好的第一步是打印出回调到控制台的响应，以了解加载的对象是如何构建的。通常在这些加载器中，几何或网格作为组的层次结构返回。理解这一点可以更容易地放置和应用正确的材料，并采取任何其他额外的步骤。此外，查看几个顶点的位置，以确定是否需要上下缩放模型以及相机的位置。在这个例子中，我们还调用了`computeFaceNormals`和`computeVertexNormals`。这是确保正确渲染所用材料(`THREE.MeshLambertMaterial`)所必需的。

下一个示例(`07-load-obj-mtl.html`)使用`OBJMTLLoader`加载模型并直接指定材质。下面的截图显示了这个例子:

![The OBJ and MTL formats](graphics/2215OS_08_14.jpg)
...
但是，正如所见，可以很容易地将复杂的模型直接加载到 Three.js 中，并在浏览器中实时渲染。不过，您可能需要微调一些材料属性。

### 加载 Collada 模型

Collada 模型(扩展名为`.dae`)是定义场景和模型(以及动画，我们将在下一章中看到)的另一种非常常见的格式。在 Collada 模型中，定义的不仅仅是几何图形，还有材料。甚至可以定义光源。
...
![Loading a Collada model](graphics/2215OS_08_15.jpg)
...
如你所见，对于大多数复杂的模型，包括材料，你经常需要采取一些额外的步骤才能得到想要的结果。通过仔细观察材料是如何配置的(使用`console.log()`)或用测试材料替换它们，问题往往很容易被发现。
...
屏幕上显示程序运行的图片
 ...
 |
| --- | --- | --- |
| STL〔t0〕 | `08-load-STL.html` | ![Loading the STL, CTM, VTK, AWD, Assimp, VRML, and Babylon models](graphics/2215OS_08_16.jpg) |
| CTM〔t0〕 | `09-load-CTM.html` | ![Loading the STL, CTM, VTK, AWD, Assimp, VRML, and Babylon models](graphics/2215OS_08_17.jpg) |
| VTK〔t0〕 | `10-load-vtk.html` | ![Loading the STL, CTM, VTK, AWD, Assimp, VRML, and Babylon models](graphics/2215OS_08_18.jpg) |
| awd〔t0〕 | `11-load-awd.html` | ![Loading the STL, CTM, VTK, AWD, Assimp, VRML, and Babylon models](graphics/2215OS_08_19.jpg) |
| asimp〔t0〕 | `12-load-assimp.html` | ![Loading the STL, CTM, VTK, AWD, Assimp, VRML, and Babylon models](graphics/2215OS_08_20.jpg) |
| VRML〔t0〕 | `13-load-vrml.html` | ![Loading the STL, CTM, VTK, AWD, Assimp, VRML, and Babylon models](graphics/2215OS_08_21.jpg) |
| 巴比伦 | 巴比伦装载机与本表中的其他装载机略有不同。使用这个加载器，您不会加载单个`THREE.Mesh`或`THREE.Geometry`实例，但是使用这个加载器，您可以加载一个完整的场景，包括灯光。`14-load-babylon.html` | ![Loading the STL, CTM, VTK, AWD, Assimp, VRML, and Babylon models](graphics/2215OS_08_22.jpg) |

如果您查看这些示例的源代码，您可能会发现对于其中一些示例，我们需要在模型正确渲染之前更改一些材质属性或进行一些缩放。我们需要这样做的原因是因为模型是在其外部应用程序中创建的，给了它不同于我们通常在三个. js 中使用的维度和分组
...
![Show proteins from Protein Data Bank](graphics/2215OS_08_23.jpg)
...
![Show proteins from Protein Data Bank](graphics/2215OS_08_24.jpg)
...
![Creating a particle system from a PLY model](graphics/2215OS_08_25.jpg)
...
如您所见，我们使用`THREE.PLYLoader`加载模型。回调返回`geometry`，我们使用这个几何图形作为`THREE.PointCloud`的输入。我们使用的材料与上一章最后一个例子中使用的材料相同。如您所见，使用 Three.js，可以非常容易地组合来自各种来源的模型，并以不同的方式渲染它们，所有这些都只需几行代码。
...
Three.js 支持大量的外部格式。当使用这些格式加载器时，查看源代码并注销回调中收到的信息是一个好主意。这将帮助您了解获得正确网格并将其设置为正确位置和比例所需的步骤。通常，当模型没有正确显示时，这是由其材质设置造成的。可能是使用了不兼容的纹理格式，不透明度定义不正确，或者该格式包含到纹理图像的不正确链接。通常，使用测试材料来确定模型本身是否被正确加载是一个好主意，并将加载的材料记录到 JavaScript 控制台，以检查意外的值。也可以导出网格和场景，但是记住三个 js 的`GeometryExporter`、`SceneExporter`、`SceneLoader`还在工作中。
