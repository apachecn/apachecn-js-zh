...
让我们在`App.re`的顶部检查到 JavaScript 的`require`函数的绑定。由于我们将在[第 4 章](4.html)、*BuckleScript、Belt和互操作性*中更深入地探讨BuckleScript，让我们推迟讨论细节，只是简单地看看这个绑定在做什么:
...
BuckleScript 允许我们使用`##`语法访问任意的 JavaScript 对象字段。我们可以在任何`Js.t`类型上使用语法，这是任意 JavaScript 对象的原因类型。我们将在[第 4 章](4.html)、*BuckleScript、Belt和互操作性*中了解更多关于这个和其他互操作性特性的信息。
