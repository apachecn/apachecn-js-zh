...
![Figure 10.1 – Network tab showing the successful installation of tfjs ](img/B17076_10_01.jpg)
...
![Figure 10.2 – Tensor output from add operation ](img/B17076_10_02.jpg)
...
![Figure 10.3 – Output from testing tfjs installed with package managers ](img/B17076_10_03.jpg)
...
在节点中安装`tfjs`。 js 很简单，但是首先要保证你的系统上安装了 node . js、`npm`或者`yarn`。
...
![Figure 10.4 – Output from testing tfjs installed in Node.js ](img/B17076_10_04.jpg)
...
![Figure 10.5 – Comparison between simple n-dimensional arrays and a tensor ](img/B17076_10_05.jpg)
...
![Figure 10.6 – Comparison of tensors with different ranks ](img/B17076_10_06.jpg)
...
![Figure 10.7 – Error thrown from shape mismatch ](img/B17076_10_07.jpg)
...
*   **矩阵**:这些运算符用于矩阵运算，如点积、范数或换位。您可以在这里看到支持的运算商的完整列表:[https://js.tensorflow.org/api/3.7.0/#Operations-Matrices](https://js.tensorflow.org/api/3.7.0/#Operations-Matrices)。
...
![Figure 10.8 – Invalid shape error when performing an operation on tensors with a different shape ](img/B17076_10_08.jpg)
...
5.  And in your terminal, run the `data_proc.js` file using the following command:
    ...
    ![Figure 10.9 – Displaying the head value of the BigMart sales dataset ](img/B17076_10_09.jpg)
...
![Figure 10.10 – Final BigMart data tensors after processing ](img/B17076_10_10.jpg)
...
![Figure 10.11 – Model training logs showing loss and root MSE ](img/B17076_10_11.jpg)
...
2.  In the `predict.js` file, add the following code:
    ...
    ![Figure 10.12 – Model summary of the saved model ](img/B17076_10_12.jpg)
...
3.  Now, create a new function called `predict` that uses the saved model to make a prediction, as shown in the following code snippet:
    ...
    在现实世界的应用中，您通常会从另一个**逗号分隔值** ( **CSV** )文件中加载一个测试数据集，并应用与我们在培训期间相同的数据处理步骤。该示例使用内联数据点，只是为了演示如何使用保存的模型进行预测。
